...
# SCP-BLOCKED as of 2026-08-15, same reason as kms.tf. Cognito's CustomEmailSender trigger, not an
...
resource "aws_lambda_function" "test_email_bypass" {
  ...
  role             = aws_iam_role.lambda_exec.arn
  handler          = "com.mootmaker.handler.CustomEmailSenderBypassHandler::handleRequest"
  runtime          = "java25"
  filename         = local.lambda_jar_path
  source_code_hash = local.lambda_jar_hash
  memory_size      = 512
  ...
  environment {
    variables = merge(local.lambda_env_vars, {
      TEST_EMAIL_CODES_TABLE_NAME   = aws_dynamodb_table.test_email_codes[0].name
      TEST_EMAIL_BYPASS_KMS_KEY_ARN = aws_kms_key.test_email_bypass[0].arn
    })
  }
}
...
resource "aws_lambda_alias" "test_email_bypass_live" {
  ...
  function_version = aws_lambda_function.test_email_bypass[0].version
}
