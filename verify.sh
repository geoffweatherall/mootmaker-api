...
# Most tests reset the database immediately before they act, via the mootmaker-tools/database-reset
...
# for this environment (mootmaker-tools/database-reset/deploy.sh <environment>).
set -euo pipefail
...
# Deterministic name computed the same way mootmaker-tools/database-reset's own run.sh does -
