...
public final class DayFixtures {
    ...
    /** Permissive enough that no test date falls outside it, so only tests about the window mention it. */
    ...
    public static List<Map<String, AttributeValue>> dayItems(final MeetingRecord... meetings) {
        final Map<String, List<MeetingRecord>> byDate = Arrays.stream(meetings)
                .collect(Collectors.groupingBy(MeetingRecord::date, LinkedHashMap::new, Collectors.toList()));
        ...
        Arrays.stream(meetings).forEach(meeting -> items.add(Map.of(
                "pk", AttributeValue.builder().s(DayRepository.POINTER_PK_PREFIX + meeting.id()).build(),
    ...
    /**
     * The config item Terraform seeds when the table is created. Tests that exercise a write path need
     * it, because the bookable window cannot be computed without it - which is the same reason a real
     * environment without one refuses every booking.
     */
     ...
     */
    public static void addMeeting(final FakeDynamoDbClient client, final String tableName, final MeetingRecord meeting) {
        final List<Map<String, AttributeValue>> items = client.tables.computeIfAbsent(tableName, _ -> new ArrayList<>());
        final String partitionKey = Day.partitionKey(meeting.date());
        final List<MeetingRecord> alreadyThere = items.stream()
        ...
        items.add(new Day(meeting.date(), 1,
                Stream.concat(alreadyThere.stream(), Stream.of(meeting)).toList()).toItem());
        items.add(Map.of(
                "pk", AttributeValue.builder().s(DayRepository.POINTER_PK_PREFIX + meeting.id()).build(),
                "date", AttributeValue.builder().s(meeting.date()).build()));
    }
    ...
    public static List<MeetingRecord> meetingsIn(final FakeDynamoDbClient client, final String tableName) {
    ...
    public static void seed(final FakeDynamoDbClient client, final String tableName,
            final String earliestRetainedDate, final MeetingRecord... meetings) {
