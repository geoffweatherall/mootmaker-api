 ...
 * DatabaseReset}/{@code DatabaseRepairHandler}'s repairs issue their per-item calls concurrently
 * (see {@code ConcurrencyUtils.runInParallel}), so every method here is synchronized - a real
 * {@code DynamoDbClient} handles concurrent calls from multiple threads fine, and this fake needs
 * to behave the same way, since a plain {@code ArrayList} silently drops entries (or throws) under
...
public class FakeDynamoDbClient implements DynamoDbClient {
   ...
   * every table here, which are all keyed by "id" - meeting-participants is the one exception, but
   * nothing under test writes to it via a bare putItem rather than transactWriteItems). Previously
  ...
  private void replace(final String tableName, final Map<String, AttributeValue> item) {
    final List<Map<String, AttributeValue>> items =
        tables.computeIfAbsent(tableName, _ -> new ArrayList<>());
  ...
  private Map<String, AttributeValue> find(
      final String tableName, final Map<String, AttributeValue> item) {
    final String keyName = item.containsKey("pk") ? "pk" : "id";
  ...
  private boolean conditionHolds(final String tableName, final Put put) {
    ...
    throw new UnsupportedOperationException(
        "FakeDynamoDbClient does not model the condition: " + condition);
  }
   ...
   */
  @Override
  public synchronized TransactWriteItemsResponse transactWriteItems(
  ...
  public synchronized GetItemResponse getItem(final GetItemRequest request) {
    final List<Map<String, AttributeValue>> items =
        tables.getOrDefault(request.tableName(), new ArrayList<>());
    return items.stream()
  ...
  /**
   * Matches on every attribute in key, not just "id" - meeting-participants is keyed by
   * personId+sortKey.
   */
  private static boolean matchesKey(
      final Map<String, AttributeValue> item, final Map<String, AttributeValue> key) {
    return key.entrySet().stream()
        .allMatch(entry -> entry.getValue().equals(item.get(entry.getKey())));
  }

  /**
   * Supports the key condition shapes the handlers under test issue against a table or GSI - a
   * single equality clause, optionally followed by " AND " and either a simple comparator (=, <,
   * <=, >, >=), a begins_with(...) call, or a BETWEEN ... AND ... range - plus an optional
   * FilterExpression of comparator clauses joined by " AND ". Attribute name tokens starting with
   * "#" are resolved via ExpressionAttributeNames, same as real DynamoDB (used for names that
   * collide with DynamoDB's reserved-word list, e.g. "bucket"). Matches by filtering the table's
   * items directly (as if every GSI were an ALL-projection copy of the base table) rather than
   * modelling indexes or partition/sort key storage, since the fake only needs to behave the same
   * as the real query, not perform like it - it also does NOT model DynamoDB's reserved-word list,
   * so (unlike real DynamoDB) it won't reject an unaliased reserved word used as a literal
   * attribute name.
   */
  @Override
  public synchronized QueryResponse query(final QueryRequest request) {
    final List<Map<String, AttributeValue>> tableItems =
        tables.getOrDefault(request.tableName(), new ArrayList<>());
    final Map<String, AttributeValue> values = request.expressionAttributeValues();
    final Map<String, String> names =
        request.expressionAttributeNames() == null ? Map.of() : request.expressionAttributeNames();

    Stream<Map<String, AttributeValue>> matches =
        tableItems.stream()
            .filter(
                item -> matchesKeyCondition(item, request.keyConditionExpression(), values, names));
    if (request.filterExpression() != null) {
      matches =
          matches.filter(
              item -> matchesFilterExpression(item, request.filterExpression(), values, names));
    }
    final List<Map<String, AttributeValue>> items = matches.toList();
    return QueryResponse.builder().items(items).count(items.size()).build();
  }

  private static boolean matchesKeyCondition(
  ...
  private static boolean matchesFilterExpression(
      final Map<String, AttributeValue> item,
  ...
  private static boolean matchesComparator(
      final Map<String, AttributeValue> item,
    ...
    for (final String operator : COMPARATORS) {
      ...
      final AttributeValue actual =
          item.get(attributeName(condition.substring(0, index).trim(), names));
      final AttributeValue expected =
          values.get(condition.substring(index + operator.length() + 2).trim());
      if (actual == null) {
        return false;
      }
  ...
  private static boolean matchesBeginsWith(
      final Map<String, AttributeValue> item,
      final String condition,
      final Map<String, AttributeValue> values,
      final Map<String, String> names) {
  ...
  private static boolean matchesBetween(
      final Map<String, AttributeValue> item,
      final String condition,
      final Map<String, AttributeValue> values,
      final Map<String, String> names) {
  ...
  /**
   * Resolves a "#alias" token via ExpressionAttributeNames; any other token is a literal attribute
   * name.
   */
  ...
  /**
   * Supports only single-table requests with no unprocessed keys, which is all the handlers under
   * test issue.
   */
  @Override
