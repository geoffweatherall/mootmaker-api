...
/**
 * The point of these is not that the numbers are right - it is that the <i>assertion</i> is real. A
 * check that only ever passes for the values compiled in cannot tell you it would catch a bad
...
class LimitsTest {
  ...
  class Configured {
    ...
    @Test
    @DisplayName(
        "a full day fits inside the DynamoDB item cap, with the modelled figure the design states")
    ...
    void produceAResponseInsideTheAppSyncCap() {
      assertTrue(
          Limits.worstCaseResponseBytes()
              <= Limits.APPSYNC_MAX_RESPONSE_BYTES * Limits.RESPONSE_SAFETY_FRACTION,
    ...
    void leaveTheStaticLimitsInsufficientOnTheirOwn() {
      final long staticWorstCase =
          (long) Limits.MAX_DATES_PER_REQUEST
              * Limits.MAX_MEETINGS_PER_DAY
              * Limits.MEETING_JSON_BYTES;
      assertTrue(
          staticWorstCase > Limits.APPSYNC_MAX_RESPONSE_BYTES,
          "the static limits now fit unaided ("
              + staticWorstCase
              + " bytes), so the fail-fast "
  ...
  class Fires {
    ...
    @Test
    @DisplayName(
        "when the dynamic cap becomes unreachable, so it would silently stop doing anything")
