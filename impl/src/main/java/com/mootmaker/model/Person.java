 ...
 * which Person is this caller? - is answered by the {@code custom:personId} claim on the token, at
 * no cost. This attribute answers only the reverse, which account deletion needs, and it is free to
 * read because deletion already holds the Person item. Being a list means a second sign-in method
 * (a federated identity provider alongside a password) is a data operation rather than a schema
...
public record Person(
  ...
  /**
   * Absent attribute means "never chose", so the default applies. An unrecognised value means the
   * stored data predates or postdates this build's enum; falling back to the default beats failing
   * the whole read for a display preference nothing downstream depends on.
   */
  private static <E> E readEnum(
      final AttributeValue stored, final Function<String, E> parse, final E fallback) {
