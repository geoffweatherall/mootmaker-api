 ...
 * list, so this converts to and from an {@code AttributeValue} rather than a whole item. The
 * {@code bucket = "ALL"} attribute went with it: that existed solely to give the deleted
 * {@code bucket-startTime-index} GSI a partition key, and every item in the table carried it.
 */
