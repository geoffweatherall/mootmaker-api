...
/**
 * Broadcasts "these dates changed" to every connected client, by calling the API's own
 * {@code publishDaysInvalidated} mutation over IAM-signed HTTP.
 *
...
public final class DaysInvalidatedPublisher implements DayBroadcaster {
     ...
     * every millisecond spent here is latency the user pays for an update meant for other people.
     * A broadcast that has not completed in two seconds is worth abandoning.
     */
    ...
    public DaysInvalidatedPublisher(final SdkHttpClient http, final AwsCredentialsProvider credentials,
                                    final URI endpoint, final Region region) {
        this.http = http;
     ...
     * backs database-reset and database-repair, which have no reason to broadcast and no grant to
     * do so.
     */
    ...
    private void send(final String body) throws IOException {
        final SdkHttpRequest unsigned = SdkHttpRequest.builder()
        ...
        final SignedRequest signed = AwsV4HttpSigner.create().sign(r -> r
                .identity((AwsCredentialsIdentity) credentials.resolveCredentials())
                .request(unsigned)
        ...
        final HttpExecuteResponse response = http.prepareRequest(HttpExecuteRequest.builder()
        ...
        final String payload = response.responseBody()
                .map(s -> {
                    try (var in = s) {
    ...
    private static String body(final Collection<String> dates) {
        final String quoted = dates.stream()
                .map(d -> '"' + d + '"')
                .collect(Collectors.joining(","));
        return "{\"query\":\"" + MUTATION.replace("\"", "\\\"") + "\",\"variables\":{\"dates\":[" + quoted + "]}}";
    }
}
