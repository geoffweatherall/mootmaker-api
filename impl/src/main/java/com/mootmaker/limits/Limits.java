 ...
 *
 * <p><b>This is layer 1 of the item-size guarantee.</b> The requirement is absolute: there must be
 * no input a user can construct that produces a DynamoDB item over 400 KB. Meeting that needs the
 * limits to be provably consistent with each other, not merely individually sensible - so {@link
 * #assertConsistent()} runs during Lambda initialisation and refuses to start if they are not.
 *
 * <p>SnapStart lands this in exactly the right place: publishing a Lambda version <i>executes
 * init</i>, so an inconsistent set of limits fails the <b>deploy</b> rather than a user's booking.
 * That is what stops the guarantee quietly decaying when someone later raises a limit, or adds a
 * field to the persisted meeting shape without revisiting the arithmetic.
 *
 * <p>Layer 2 is per-request validation. Layer 3 measures the actual serialised item immediately
 * before the write, and is the backstop that holds even if the byte model below is wrong - and it
 * will drift, because it is an estimate of DynamoDB's own accounting rather than a reading of it.
 */
public final class Limits {
   ...
   * measured for real immediately before it is written (layer 3), so if this byte model is
   * optimistic the consequence is a premature "day is full" rejection - never an oversized item.
   * The guarantee holds on measurement, not on the model, so budgeting the model below 100% would
   * buy nothing.
   *
   * <p>Nothing measures the real response at run time. The dynamic meetings-per-response cap counts
   * meetings and multiplies by the modelled size, so there the model IS the guarantee - and a
   ...
   *
   * <p>Not in the original design, and it exists for a specific reason: the day item and the {@code
   * PTR#} pointer for every meeting it gains are written in ONE {@code TransactWriteItems}, and
   * DynamoDB caps that at 100 items. One day item plus 99 pointers is exactly 100. Without this
  ...
  record Budget(
    ...
    int worstCaseResponseBytes() {
      return maxRooms * ROOM_JSON_BYTES
          + maxPeople * PERSON_JSON_BYTES
          + maxMeetingsPerResponse * MEETING_JSON_BYTES;
    }
  }

  /** The limits this build actually enforces. */
  static final Budget CONFIGURED =
      new Budget(
  ...
  static void assertConsistent(final Budget budget) {
    require(
        budget.maxMeetingsPerDay() > 0
            && budget.maxAttendees() > 0
            && budget.maxSubjectBytes() > 0
            && budget.maxRooms() > 0
            && budget.maxPeople() > 0
            && budget.maxDates() > 0
    ...
    require(
        budget.worstCaseDayItemBytes() <= DYNAMODB_MAX_ITEM_BYTES,
        "a day at every limit is "
            + budget.worstCaseDayItemBytes()
            + " bytes, over the "
    ...
    require(
        budget.worstCaseResponseBytes() <= responseBudget,
        "a response at every limit is "
            + budget.worstCaseResponseBytes()
            + " bytes, over the "
            + responseBudget
            + "-byte budget. Lower the meetings-per-response, rooms or people limit.");
    ...
    require(
        (long) budget.maxDates() * budget.maxMeetingsPerDay() > budget.maxMeetingsPerResponse(),
    ...
    require(
        1 + MAX_MEETINGS_PER_BULK_CREATE <= DYNAMODB_MAX_TRANSACT_ITEMS,
        "one day item plus MAX_MEETINGS_PER_BULK_CREATE pointers exceeds DynamoDB's "
            + DYNAMODB_MAX_TRANSACT_ITEMS
            + "-item transaction cap, so a full bulk create would "
            + "fail at the write rather than at validation.");

    final long subscriptionBudget =
        (long) (APPSYNC_MAX_SUBSCRIPTION_BYTES * RESPONSE_SAFETY_FRACTION);
    require(
        (long) budget.maxDates() * DATE_JSON_BYTES <= subscriptionBudget,
