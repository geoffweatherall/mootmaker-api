 ...
 * small tables keyed by a plain {@code id}, and neither has invariants of its own worth guarding
 * the way a day item does. Package-private on purpose: this is plumbing for the repositories, not a
        ...
        .map(
            chunkIds ->
                CompletableFuture.supplyAsync(
                    () -> fetchChunk(dynamoDbClient, tableName, chunkIds)))
        .toList()
        .stream()
