 ...
 * implementation, and no expectation of ever swapping DynamoDB out. It is an instance rather than
 * the static utilities elsewhere in this package so that it owns its client, table name and retry
 * budget - an invariant cannot be bypassed by calling a static method with different arguments.
 *
...
public final class DayRepository {
  ...
  /**
   * The single config item, holding the retention boundary. Seeded by Terraform when the table is
   * created.
   */
  ...
  public DayRepository(
      final DynamoDbClient dynamoDbClient, final String tableName, final Clock clock) {
        ...
        if (attempt == MAX_WRITE_ATTEMPTS) {
          throw new IllegalStateException(
              "Could not write day "
                  ...
                  + " after "
                  + MAX_WRITE_ATTEMPTS
                  + " attempts - sustained write contention on one date.",
              e);
        }
      }
    }
  ...
  private List<TransactWriteItem> writeItems(
    ...
    final Set<String> after =
        next.meetings().stream().map(MeetingRecord::id).collect(Collectors.toSet());

    after.stream()
        .filter(id -> !before.contains(id))
        .forEach(
            id -> items.add(TransactWriteItem.builder().put(pointerPut(id, next.date())).build()));
    before.stream()
            ...
            .getItem(
                GetItemRequest.builder()
                    ...
                    .key(
                        Map.of(
                            "pk",
                            AttributeValue.builder().s(POINTER_PK_PREFIX + meetingId).build()))
                    .consistentRead(true)
                    .build())
            .item();
  ...
  public void advanceBoundaryTo(final String monday) {
    ...
    if (monday.compareTo(current.earliestRetainedDate()) < 0) {
      throw new IllegalArgumentException(
          "Refusing to move the retention boundary backwards, from "
              + current.earliestRetainedDate()
              + " to "
              + monday
   ...
   * here for {@code database-reset} alone rather than being offered as a general operation. Moving
   * the boundary back normally advertises history that has already been deleted; once the table
   * holds no history at all, there is nothing left to advertise falsely, and every day in the new
   * window is legitimately empty rather than missing.
   *
   * <p>Needed because the boundary is the one piece of state a reset could not previously undo. The
   * acceptance suite advances it deliberately, to observe a deletion at all, and without this that
   * advance was permanent: an environment's bookable window would creep further into the future
   * with every run until nothing near-term could be booked. Preserving the config item was always
   * right - deleting it breaks every write - but preserving its VALUE was not.
   */
  ...
  public List<String> deleteDaysBefore(final String boundary) {
    final List<Day> expired =
        scanDays().stream().filter(day -> day.date().compareTo(boundary) < 0).toList();
              ...
              .delete(
                  Delete.builder()
                      ...
                      .key(
                          Map.of(
                              "pk",
                              AttributeValue.builder().s(Day.partitionKey(day.date())).build()))
                      .build())
              .build());
      day.meetings()
          .forEach(
  ...
  /**
   * Every day item in the table, for the rare operations that genuinely need all of them - account
   * deletion, and the cleanup job. Cheap by construction: the horizon and retention bound the table
   * at 217 day items, which is why this replaced the meeting-participants join table rather than an
   * index replacing it.
   */
