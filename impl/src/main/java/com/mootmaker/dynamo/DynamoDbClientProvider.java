...
/**
 * Lazily-built singleton, reused across warm Lambda invocations to avoid repeated client setup
 * cost.
 */
public final class DynamoDbClientProvider {
  ...
  /**
   * Loads/JIT-warms the DynamoDB SDK's marshallers, HTTP client internals, and credential/ signing
   * chain by making one real, side-effect-free DescribeTable call - the same cost a handler's first
   * real request would otherwise pay at interpreter speed. Called eagerly from {@link #client()}
   * above (baking it into the SnapStart snapshot, since it runs before checkpoint) and again from
   * {@link AfterRestorePriming} (since the TCP/TLS connection itself never survives a
   * snapshot/restore cycle regardless of what's primed - see AWS's SnapStart networking guidance -
   * so it always needs re-establishing post-restore, just cheaply now that the classes are already
   * loaded). ROOMS_TABLE_NAME is set for every handler regardless of whether it uses the rooms
   * table (see lambda.tf's lambda_env_vars), so any table name primes the same shared client
   * equally well.
   */
   ...
   * fires again each time a new execution environment resumes from the same snapshot, which is why
   * this half can't just be more eager code the way {@link #client()}'s first {@code
   * primeConnection} call is.
   */
