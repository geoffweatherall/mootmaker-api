 ...
 *
 * <p>Following AWS's documented rules: an attribute costs its name plus its value; strings and
 * binary cost their raw length; a number costs roughly one byte per two significant digits plus
 * one; lists and maps carry three bytes of overhead plus one byte per element, and a map's element
 * names count too. It is close rather than exact - which is fine, because it is only ever used with
 * headroom against the cap, never to predict a value someone depends on.
 */
