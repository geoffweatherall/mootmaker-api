 ...
 *
 * <p>One call, one item write. mootmaker-demo-data's per-meeting fan-out collapses into one call per
 * seeded day, which is also what stops its parallel creates colliding on the day item's version.
 *
...
public class CreateMeetingsHandler implements RequestHandler<Map<String, Object>, Object> {
     ...
     */
    private void write(final String date, final List<MeetingRecord> accepted, final List<Map<String, Object>> failures) {
        try {
            days.mutate(date, day -> {
                if (day.meetings().size() + accepted.size() > Limits.MAX_MEETINGS_PER_DAY) {
                    throw new MeetingRejected(MeetingError.DayIsFull);
                }
                return day.withMeetings(Stream.concat(day.meetings().stream(), accepted.stream()).toList());
            });
            ...
            LOGGER.error("Byte model drift: bulk write to day {} was rejected by the write-time size check", date, e);
    ...
    private static void failuresForEveryAccepted(final List<MeetingRecord> accepted,
            final List<Map<String, Object>> failures, final List<String> errors) {
    ...
    private List<String> errorsFor(final Map<String, Object> input, final String date,
            final Boundaries boundaries, final List<MeetingRecord> soFar) {
    ...
    private static MeetingRecord toRecord(final Map<String, Object> input) {
        ...
        return new MeetingRecord(UUID.randomUUID().toString(), (String) input.get("roomId"),
                (String) input.get("organiserId"), attendeeIds,
                com.mootmaker.limits.Subjects.normalise((String) input.get("subject")),
                canonical((String) input.get("startTime")), canonical((String) input.get("endTime")));
    }
    ...
    private Map<String, Object> dayResponse(final String date) {
        ...
        response.put("meetings", days.read(date).meetings().stream()
    ...
    private static Map<String, Object> rejectedWholeCall(final int size) {
        final Map<String, Object> result = new HashMap<>();
        result.put("day", null);
        result.put("failures", List.of(Map.of("index", 0,
                "errors", List.of(MeetingError.TooManyMeetingsInOneCall.name()))));
        LOGGER.warn("Rejected a bulk create of {} meetings; the limit is {}", size, Limits.MAX_MEETINGS_PER_BULK_CREATE);
        return result;
    }
}
