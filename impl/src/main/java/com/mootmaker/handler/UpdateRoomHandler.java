...
/**
 * AppSync direct-Lambda resolver for {@code Mutation.updateRoom}. Admin only - see {@link
...
public class UpdateRoomHandler implements RequestHandler<Map<String, Object>, Object> {
  ...
  public UpdateRoomHandler() {
    this(
        DynamoDbClientProvider.client(), System.getenv().getOrDefault("ROOMS_TABLE_NAME", "Rooms"));
  }
  ...
  private List<Map<String, Object>> allRooms() {
    return new RoomRepository(dynamoDbClient, tableName)
        .listAll().stream().map(Room::toResponseMap).toList();
  }
}
