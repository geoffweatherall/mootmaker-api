 ...
 * retrying. This finds every confirmed Cognito user whose {@code custom:personId} claim is missing,
 * or points at a Person that no longer exists, and creates one, named after the part of their email
 * before the "@" - a reasonable one-off backfill default, <b>not</b> a re-implementation of the
 * trigger's own behaviour (which uses the Cognito {@code name} attribute, unavailable here for
 * users - like the demo/e2e ones - who never had one set).
 */
final class CreateMissingPersonsRepair {
  ...
  static Result run(
    ...
    // Each user's check-and-create is independent of every other user's, so they run on the shared
    // bounded thread pool rather than one at a time -
    ...
    ConcurrencyUtils.runInParallel(
        ...
        user -> {
          ...
          final Optional<String> claimedPersonId =
              optionalAttribute(user, Identity.PERSON_ID_CLAIM);

          // Three states, and the middle one is the reason the trigger sets the claim before
          // writing
          // the Person: a claim pointing at a Person that does not exist is repairable EXACTLY,
          // using
   ...
   */
  private static List<UserType> listConfirmedUsers(
      final CognitoIdentityProviderClient client, final String userPoolId) {
