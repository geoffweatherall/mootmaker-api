 ...
 * trigger, which already fires exactly once per confirmed sign-up before the user's first
 * sign-in/token, rather than adding a separate {@code PreSignUp} trigger (which has no reliable way
 * to inject an attribute value the way this trigger's Admin API access does). Unlike the AppSync
 * resolver handlers in this package, Cognito requires trigger Lambdas to return the event
 ...
 * logged and swallowed rather than thrown, to avoid blocking sign-up.
 */
public class PostConfirmationCreatePersonHandler
    implements RequestHandler<Map<String, Object>, Map<String, Object>> {

  private static final Logger LOGGER =
      LoggerFactory.getLogger(PostConfirmationCreatePersonHandler.class);
   ...
   * on every confirmation event for this trigger source rather than being gated on whether {@link
   * #createPersonIfNeeded} actually created something this time.
   */
  private void setDefaultClassIfNeeded(final Map<String, Object> event) {
    ...
    cognitoClient.adminUpdateUserAttributes(
        AdminUpdateUserAttributesRequest.builder()
            ...
            .username(cognitoSub)
            .userAttributes(
                AttributeType.builder().name("custom:class").value(DEFAULT_CLASS).build())
            .build());
    LOGGER.info(
        "Set default class '{}' for confirmed sign-up (sub '{}')", DEFAULT_CLASS, cognitoSub);
  }
