 ...
 * {@code DayRepository.mutate}'s change function, which must return a day. Throwing both reports
 * the failure and abandons the write, which is exactly the behaviour wanted: a day that is full, or
 * a room that has just been taken, must not be written.
 *
 * <p>These are validation results, not faults. Callers turn them into the typed {@code errors}
 * array the schema promises - a client renders them, and none of them is ever a 500.
 */
