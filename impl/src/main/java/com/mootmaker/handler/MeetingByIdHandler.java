 ...
 * date, then that day - which is the entire reason the pointer is written in the same transaction as
 * the day item.
 *
 * <p><b>Not found and aged-out-of-retention return the same answer.</b> That is deliberate: callers
 * handle one case instead of two, and nothing confirms that a given id was once valid. A shared link
 * to a meeting older than retention simply stops resolving.
 */
public class MeetingByIdHandler implements RequestHandler<Map<String, Object>, Object> {
    ...
    MeetingByIdHandler(final DynamoDbClient dynamoDbClient, final String meetingsTableName,
    ...
    MeetingByIdHandler(final DayRepository days, final RoomRepository rooms, final PersonRepository people) {
        this.days = days;
        this.rooms = rooms;
        this.people = people;
    }
    ...
    public Object handleRequest(final Map<String, Object> event, final Context context) {
        ...
        final String id = arguments instanceof Map<?, ?> args ? (String) ((Map<String, Object>) args).get("id") : null;
        ...
        final Optional<MeetingRecord> found = days.findDateOfMeeting(id)
                .map(days::read)
        ...
        final Map<String, Room> roomsById = resolveRooms ? rooms.loadByIds(Set.of(record.roomId())) : Map.of();
        final Map<String, Person> peopleById = resolvePeople
                ? people.loadByIds(Stream.concat(Stream.of(record.organiserId()), record.attendeeIds().stream())
