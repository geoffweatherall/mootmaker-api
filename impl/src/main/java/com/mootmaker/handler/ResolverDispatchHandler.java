 ...
 *
 * <p>Routes on AppSync's {@code $context.info} ({@code fieldName}, {@code parentTypeName}), which
 * the shared request template in {@code appsync.tf} serialises explicitly. It used to be a
 * pass-through ({@code $util.toJson($ctx)}) that included those for free; it now names every field
 * it sends, because {@code selectionSetList} is not serialised unless it is asked for by name.
 *
 * <p>The constructor runs {@link Limits#assertConsistent()} before anything else - a statement
 * before {@code this(...)}, which Java 25's flexible constructor bodies allow. That placement is
 * deliberate: SnapStart executes init when a version is published, so inconsistent limits fail the
 * deploy.
 */
public class ResolverDispatchHandler implements RequestHandler<Map<String, Object>, Object> {
  ...
  public ResolverDispatchHandler() {
    ...
    this(
        Map.ofEntries(
            ...
            Map.entry("Mutation.deleteRoom", new DeleteRoomHandler()),
            Map.entry("Mutation.createPerson", new CreatePersonHandler()),
            Map.entry("Mutation.updateMyName", new UpdateMyNameHandler()),
            Map.entry("Mutation.renamePerson", new RenamePersonHandler()),
            Map.entry("Mutation.setPersonAdmin", new SetPersonAdminHandler()),
            Map.entry("Mutation.deletePerson", new DeletePersonHandler()),
            ...
            Map.entry("Mutation.updateMeeting", new UpdateMeetingHandler()),
            Map.entry("Mutation.cancelMeeting", new CancelMeetingHandler()),
  ...
  ResolverDispatchHandler(
      final Map<String, RequestHandler<Map<String, Object>, Object>> handlersByRoutingKey) {
  ...
  public Object handleRequest(final Map<String, Object> event, final Context context) {
    ...
    final RequestHandler<Map<String, Object>, Object> handler =
        handlersByRoutingKey.get(routingKey);
    if (handler == null) {
      throw new IllegalStateException(
          "Bad Request: no resolver registered for '" + routingKey + "'");
    }
  ...
  private static String routingKey(final Map<String, Object> event) {
    ...
    if (!(infoRaw instanceof Map<?, ?>)) {
      throw new IllegalStateException(
          "Bad Request: event has no 'info' - not an AppSync resolver invocation");
    }
