...
/**
 * Builds the GraphQL response shape for a meeting, resolving room, organiser and attendees only when
 * the query actually asked for something beyond their ids.
 *
 ...
 * SelectionSet} reports "no lookup needed" only when everything selected can be answered from the id.
 * If those two ever disagree, a null reaches a non-null field and GraphQL nulls the person, then the
 * list, then the meeting - silent data loss rather than a degraded response. One implementation means
 * they cannot drift apart per call site.
 */
final class MeetingResponse {
    ...
    static Map<String, Object> of(final MeetingRecord record, final Map<String, Room> roomsById,
        ...
        map.put("room", roomsResolved ? resolveRoom(record.roomId(), roomsById) : idOnly(record.roomId()));
     ...
     */
    private static Map<String, Object> resolvePerson(final String personId, final Map<String, Person> peopleById) {
    ...
    private static Map<String, Object> resolveRoom(final String roomId, final Map<String, Room> roomsById) {
        final Room room = roomsById.get(roomId);
        return room == null ? new Room(roomId, "Deleted room", 0).toResponseMap() : room.toResponseMap();
    }
}
