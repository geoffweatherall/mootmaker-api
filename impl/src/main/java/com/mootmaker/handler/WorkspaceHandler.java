 ...
 *
 * <p><b>Nothing not selected is fetched.</b> A calendar paging to a new week selects only
 * {@code days}, so this does no people, rooms or boundaries work at all; a reference-data refresh
 * omits {@code dates} entirely and reads no day items. That is the whole reason a composite field
 * costs no more than the narrow ones it replaced - see {@link SelectionSet} for why the test errs
 * toward fetching rather than enumerating what is expensive.
 */
public class WorkspaceHandler implements RequestHandler<Map<String, Object>, Object> {
    ...
    WorkspaceHandler(final DynamoDbClient dynamoDbClient, final String meetingsTableName,
    ...
    WorkspaceHandler(final DayRepository days, final RoomRepository rooms, final PersonRepository people) {
        this.days = days;
        this.rooms = rooms;
        this.people = people;
    }
    ...
    public Object handleRequest(final Map<String, Object> event, final Context context) {
        ...
        final CompletableFuture<Map<String, Object>> boundariesFuture =
                selection.selects("boundaries") ? async(() -> days.boundaries().toResponseMap()) : completed(null);
    ...
    private static List<String> requestedDates(final Map<String, Object> event) {
        ...
        if (list.size() > Limits.MAX_DATES_PER_REQUEST) {
            throw new IllegalArgumentException("Too many dates requested: " + list.size()
                    + " exceeds the limit of " + Limits.MAX_DATES_PER_REQUEST + ".");
        }
     ...
     */
    private List<Map<String, Object>> daysResponse(final List<String> dates, final SelectionSet selection) {
        final boolean resolveRooms = selection.needsLookup("days/meetings/room");
        final boolean resolvePeople = selection.needsLookup("days/meetings/organiser")
        ...
        for (final Day day : days.read(dates)) {
            ...
            if (meetingCount > Limits.MAX_MEETINGS_PER_RESPONSE) {
                throw new IllegalArgumentException("The response would be too large: more than "
                        + Limits.MAX_MEETINGS_PER_RESPONSE + " meetings across the requested dates. "
                        + "Ask for fewer dates.");
            }
            fetched.add(day);
        }

        final List<MeetingRecord> all = fetched.stream().flatMap(day -> day.meetings().stream()).toList();
        final Map<String, Room> roomsById = resolveRooms
                ? rooms.loadByIds(all.stream().map(MeetingRecord::roomId).collect(Collectors.toSet()))
                : Map.of();
        final Map<String, Person> peopleById = resolvePeople
                ? people.loadByIds(all.stream()
                        .flatMap(m -> Stream.concat(Stream.of(m.organiserId()), m.attendeeIds().stream()))
                        .collect(Collectors.toSet()))
                : Map.of();

        return fetched.stream().map(day -> {
            final Map<String, Object> response = new HashMap<>();
            response.put("date", day.date());
            response.put("meetings", day.meetings().stream()
                    .map(record -> MeetingResponse.of(record, roomsById, peopleById, resolveRooms, resolvePeople))
                    .toList());
            return response;
        }).toList();
    }
    ...
    private Map<String, Object> meResponse(final Map<String, Object> event) {
        return Identity.personId(event).flatMap(people::findById).map(Person::toResponseMap).orElse(null);
    }
     ...
     *
     * <p>Not cosmetic: the response bound is enforced while days accumulate, which happens on a worker
     * thread, and a plain {@code join()} would wrap it. The client would then get a generic failure
     * instead of the message telling them to ask for fewer dates - turning a rule they can act on into
     * an apparent server error.
     */
