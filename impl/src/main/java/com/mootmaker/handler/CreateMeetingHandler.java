 ...
 *
 * <p>Validation happens in two places, and the split is the important part of this class. Rules
 * that depend only on the request - times, existence, capacity, lengths, the bookable window - are
 * checked once, up front. Rules that depend on the <i>state of the day</i> - is the room still
 * free, is the day now full - are checked <b>inside</b> the read-modify-write, so a booking that
 * loses a version conflict re-validates against the day as it now is rather than as it was when the
 * request arrived.
 *
 * <p>Getting that wrong is the classic form of this bug: two concurrent bookings both read a day
 * holding 319 meetings, both conclude there is room, and both write. The conditional write makes
 * one of them retry; re-running the check on retry is what makes the retry mean anything.
 */
public class CreateMeetingHandler implements RequestHandler<Map<String, Object>, Object> {
  ...
  Meeting create(final Map<String, Object> meetingInput, final Boundaries boundaries) {
    final MeetingValidator.Validated validated =
        validator.validateRequest(meetingInput, boundaries);
