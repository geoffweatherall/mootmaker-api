...
| `updateMeeting(id, meeting)` | Mutation | **The meeting's organiser, or admin.** Full field replacement, same shape as `updateRoom`/`updatePerson` - every field in `MeetingInput` is applied, including reassigning the organiser or attendees. A continuing attendee keeps their existing `AttendeeStatus`; only a newly added one starts at `NoResponse`. Moving a meeting to a different calendar date is supported (see [Editing, cancelling, and moving a meeting to a different day](#editing-cancelling-and-moving-a-meeting-to-a-different-day)). Returns `UpdateMeetingResult` (`MeetingNotFound` if `id` no longer resolves to any meeting) |
