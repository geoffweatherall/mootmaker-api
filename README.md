...
- **Room** — `id`, `name`, `capacity`, `color`. Capacity is the total number of people the room holds (organiser + attendees). `color` is one of a fixed 8-hue palette (see the `RoomColor` enum), optional and never assigned automatically server-side — a client falls back to its own deterministic assignment when it's null, the same way every room's colour was computed before this field existed.
- **Person** — `id`, `name`, `isAdmin`, `linkedEmails`, `dateFormat`, `timeFormat`, `weekStart`. The three preferences are the person's own display preferences (`Usa`/`British`/`Iso`, `TwentyFourHour`/`AmPm`, and `Monday`/`Sunday`), non-null over GraphQL and defaulting to `Iso`/`TwentyFourHour`/`Monday` for anyone who has never chosen — including every Person written before the preferences existed, and every guest. They are **display-only**: this API accepts and returns ISO-8601 local date-times regardless of anyone's setting, and nothing server-side branches on them (see [Date/time display preferences](#datetime-display-preferences)). Also has backend-only `cognitoSubs`/`cognitoEmails` attributes, not exposed as such over GraphQL (`cognitoEmails` is surfaced as `linkedEmails`; `cognitoSubs` never is): set for a Person created automatically on sign-up (see [Sign-up creates a linked Person](#sign-up-creates-a-linked-person)), and empty for people added directly (e.g. guests with no login), so account deletion can find and remove the Person linked to a deleted Cognito user.
- **Meeting** — `id`, `room`, `organiser` (a Person), `attendees` (list of `Attendee`, each pairing a Person with their own `AttendeeStatus`: `Going`/`NotGoing`/`Maybe`/`NoResponse`), `subject`, `startTime`, `endTime`. `subject` must not be null or blank. Times are ISO-8601 local date-times with no time-zone offset (`java.time.LocalDateTime` semantics), e.g. `2026-07-01T14:30:00`, must fall on a 15-minute boundary, and `startTime`/`endTime` must fall on the same calendar date — a meeting cannot span midnight (see [Validation](#validation)). Every attendee starts at `NoResponse` and may change only their own status, via `Mutation.respondToMeeting(meetingId, status)` (self-only, no id argument, no admin override — see `updateMyPreferences` for the same pattern). The organiser has no status of their own: scheduling a meeting is treated as confirming attendance, so they are never in `attendees` at all (see `OrganiserIsAttendee` below) and there is nothing for them to set.
...
Meetings are still stored **normalised** inside the day item: a meeting holds `roomId`, `organiserId` and `attendeeIds`, never the room or person objects, so renaming a room or a person is reflected everywhere with no propagation step. `attendeeStatuses` is a parallel list to `attendeeIds` — same length and order, index `i` of one is index `i` of the other, stored as single-character codes (`G`/`N`/`M`/`U`) — never a combined list of maps, and never updated independently of `attendeeIds` (see designs/attendee-response-status.md).
...
#### Editing, cancelling, and moving a meeting to a different day

`updateMeeting`/`cancelMeeting` reuse `DayRepository.mutate`'s existing read-modify-write retry loop for a same-day change - a version conflict re-validates against the day as it now is, and re-checks the meeting still exists, on every retry, same as `createMeeting`. Editing a meeting's date to a different day needed a genuinely new capability, `DayRepository.moveMeeting`: the existing pointer write (`pointerPut`) is conditional on the pointer *not already existing* - collision safety for a freshly allocated id - which means re-adding an *existing* id to a different day via the ordinary path collides with that same id's own still-live pointer at the old date, identically on every retry. `moveMeeting` instead does two separately committed writes: add the record to the new day (with an `Update`, not a `Put`, repointing the existing `PTR#` item, conditional on it still pointing at the old date), then remove it from the old day, touching no pointer there at all. Deliberately add-then-remove: a failure between the two leaves the meeting visible on *both* days - an overcount, harmless, and safe for a retry to finish cleaning up - never gone from both, which nothing could recover. Idempotent by construction: it starts by reading the pointer itself, and only performs whichever of the two steps hasn't already committed.

An edit's room-availability and day-full checks exclude the meeting's own current slot - `RoomAvailability.isFreeIgnoring` (which already existed, unused, before this) rather than `isFree` - so editing a meeting's time within its own room, even to a range that overlaps its own prior slot, doesn't reject as a self-conflict. `Query.suggestRoom` takes the same optional `excludingMeetingId` for the identical reason, so "Suggest a room" during an edit doesn't drop the meeting's own current room from its candidates either.

`cancelMeeting` is a hard delete: the meeting is removed from its day's list entirely, and the existing pointer-diff in `DayRepository.mutate`'s write path deletes its `PTR#` row automatically, in the same transaction, with no new code. There is no cancelled/tombstone state anywhere - once gone, `meeting(id:)` returns `null`, indistinguishable from an id that never existed.

An edit that doesn't touch the attendee list preserves every continuing attendee's existing `AttendeeStatus` rather than resetting it to `NoResponse` - `UpdateMeetingHandler` looks up each continuing attendee's prior status from the record being replaced; only a genuinely new attendee starts at `NoResponse`.
...
| `updateRoom(id, room)` | Mutation | **Admin only.** Replaces a room's name/capacity/colour. Returns `UpdateRoomResult` (including `RoomNotFound`) |
| `createPerson(name)` | Mutation | **Admin only.** Returns `CreatePersonResult`. Rejects `NameAlreadyExists` if another Person already has this name (case/whitespace-insensitive) - see [mootmaker-api#70](https://github.com/geoffweatherall/mootmaker-api/issues/70) |
| `updateMyName(name)` | Mutation | **Self only, no admin override.** Renames the caller and propagates to Cognito for a linked account (see [Denormalised data](#denormalised-data-cognitos-name-attribute)) |
| `renamePerson(id, name)` | Mutation | **Admin only.** The admin-invoked counterpart to `updateMyName` - split into two mutations rather than one self-or-admin `updatePerson` so a mutation that can rename anyone is named, and findable, as exactly that |
| `setPersonAdmin(id, isAdmin)` | Mutation | **Admin only.** Grants or revokes admin access, propagating `custom:class` to every linked Cognito account. Rejects `NoLinkedAccount` for a guest Person (nothing to actually grant) and `CannotRevokeOwnAdminAccess` for a caller targeting their own Person with `isAdmin: false`. Result carries `cognitoSyncFailed: Boolean!` for a partial Cognito-sync failure |
...
| `updateMeeting(id, meeting)` | Mutation | **The meeting's organiser, or admin.** Full field replacement, same shape as `updateRoom` - every field in `MeetingInput` is applied, including reassigning the organiser or attendees. A continuing attendee keeps their existing `AttendeeStatus`; only a newly added one starts at `NoResponse`. Moving a meeting to a different calendar date is supported (see [Editing, cancelling, and moving a meeting to a different day](#editing-cancelling-and-moving-a-meeting-to-a-different-day)). Returns `UpdateMeetingResult` (`MeetingNotFound` if `id` no longer resolves to any meeting) |
| `cancelMeeting(id)` | Mutation | **The meeting's organiser, or admin.** A hard delete - the meeting is removed entirely, no cancelled/tombstone state kept. No restriction on whether it's in the past, in progress, or upcoming. Returns `CancelMeetingResult` (`day`, and `MeetingNotFound` if already gone) |
...
This runs **after** email confirmation rather than before (a `PreSignUp` trigger would fire while the address is still unverified, risking orphaned Person records for abandoned or typo'd sign-ups) or from the browser (a client-side call after `confirmSignUp()` would leave a confirmed account with no Person if the tab closes or the network drops before that call completes). Cognito also retries `PostConfirmation` invocations on failure, so the handler is idempotent — it checks the `custom:personId` claim before writing, and skips creation if a Person is already linked. Because Cognito treats an exception thrown here as a failure of the user's confirm-sign-up call (even though the account is already confirmed by that point), the handler logs and swallows any error rather than throwing, so a transient DynamoDB problem never blocks sign-up.

A separate, narrower `PreSignUp` trigger ([PreSignUpNameCollisionHandler](impl/src/main/java/com/mootmaker/handler/PreSignUpNameCollisionHandler.java)) does exist, added for a different job than Person creation: rejecting a sign-up outright when its `name` collides (case/whitespace-insensitively) with an existing Person (see [mootmaker-api#70](https://github.com/geoffweatherall/mootmaker-api/issues/70)). This doesn't contradict the reasoning above - it doesn't need email to be verified, only to compare a name against DynamoDB, and throwing from `PreSignUp` is the one point in the flow where that's the *intended* behaviour: Cognito surfaces the exception as the sign-up call's own failure, before any user is created at all, unlike `PostConfirmation`'s swallow-and-log.
...
Every Cognito user has a `custom:class` attribute, `standard` or `admin`, included in the ID token as the `custom:class` claim. `PostConfirmationCreatePersonHandler` (see [Sign-up creates a linked Person](#sign-up-creates-a-linked-person) above) sets it to `standard` for every new sign-up via `AdminUpdateUserAttributes`, right after creating the linked Person. Besides that, the only way it ever changes is `setPersonAdmin` (admin only) — the client is never trusted to set its own class directly, and the webapp's `mootmaker-webapp` app client is deliberately not granted write access to `custom:class` (see its `write_attributes` in [cognito.tf](deploy/terraform/cognito.tf)), so a signed-in user can't self-promote by calling Cognito's own attribute-update API directly. The Terraform-managed demo user is `admin` and the e2e test user is `standard` (`aws_cognito_user.demo` / `aws_cognito_user.e2e` in [cognito.tf](deploy/terraform/cognito.tf)).

`Person.isAdmin` is a **read-optimised DynamoDB copy** of the same fact, written by `setPersonAdmin` in the same call that updates `custom:class` — so the Persons admin screen can render an "Admin" badge for everyone without a live Cognito lookup per person. `custom:class` on the token stays the actual authorization source every request checks; `Person.isAdmin` never gates anything by itself.

[Identity.requireAdmin](impl/src/main/java/com/mootmaker/handler/Identity.java) is the enforcement point, checked before any logic runs in an admin-only handler (`CreateRoomHandler`, `UpdateRoomHandler`, `DeleteRoomHandler`, `CreatePersonHandler`, `RenamePersonHandler`, `SetPersonAdminHandler`, `DeletePersonHandler`) — same shape as `Identity.requireAuthenticated`, but also accepting a caller whose `scope` claim contains the `mootmaker-api/admin` OAuth scope, so the M2M `mootmaker-acceptance-tests` and `mootmaker-demo-data` clients keep working without a real Cognito user or `custom:class` claim behind them. `UpdateMeetingHandler`/`CancelMeetingHandler` use the softer `Identity.isAdmin` instead, for a meeting's organiser: existence is checked first (a bad id reports `MeetingNotFound`, never leaking a 403 for a nonexistent id), then the *current* record's organiser is resolved to a `Person` and checked against `identity.sub`, before any field-level validation runs. `updateMyName`/`updateMyPreferences`/`respondToMeeting`/`deleteMyAccount` take neither check — they're self-only, always resolved from the caller's own `custom:personId` claim, with no admin override at all. As with `updateRoom`, an authorization failure is a thrown exception (`IllegalStateException`, surfaced by AppSync as a top-level GraphQL error) rather than a typed entry in an `errors` array — this codebase already treats "not allowed to do this at all" and "allowed, but the input was invalid" as two different channels.
...
**Schema-level authorization directives were considered and rejected.** AppSync's `@aws_cognito_user_pools(cognito_groups: [...])` only checks Cognito User Pool *Group* membership, which this project has never used (admin is a custom attribute, not a group) — and structurally can't cover the M2M clients above at all, since a `client_credentials` token has no Cognito user behind it and therefore can never belong to a group. Every admin-only mutation needs to stay callable by those clients, so declarative group-based gating isn't viable here regardless of effort invested; `Identity.requireAdmin` stays the sole enforcement mechanism.

### Denormalised data: Cognito's `name` attribute

Meetings aren't denormalised by room/person name at all — see [Storage](#storage) above — so a rename via `updateRoom`/`updateMyName`/`renamePerson` is reflected everywhere automatically with no extra propagation. The one real exception is Cognito's own `name` user attribute, a separate copy of a linked person's name set once at sign-up. `UpdateMyNameHandler` (self) and `RenamePersonHandler` (admin) both keep it in sync: whenever the target person has a linked account, each calls `AdminUpdateUserAttributes` to set Cognito's `name` to match, after the DynamoDB write succeeds. This call is best-effort (logged and swallowed on failure, like the PostConfirmation trigger) so a transient Cognito problem never fails the rename itself; the DynamoDB `Person.name` remains the source of truth read by `workspace { me }`, so a swallowed sync failure only means the *next* sign-in's JWT `name` claim is briefly stale, not that the rename was lost. `SetPersonAdminHandler` follows the same DynamoDB-then-Cognito order for `custom:class`, but does **not** swallow that failure the same way — see [User classes and authorization](#user-classes-and-authorization) above and the schema's own doc comment on `SetPersonAdminResult.cognitoSyncFailed`: unlike a display name, a stuck `custom:class` sync means the grant hasn't actually taken effect yet.
...
A `Person` carries a `dateFormat`, a `timeFormat`, and a `weekStart`, set by their owner in the webapp's Settings page. The two formats are used by clients to render and parse date/times for humans; `weekStart` (`Monday`/`Sunday`) only affects which day a date-picker calendar grid starts its weeks on.
...
- `CreateRoomResult { room, errors: [RoomError!]! }` / `UpdateRoomResult { room, errors: [RoomError!]! }` / `DeleteRoomResult { rooms, errors: [RoomError!]! }`
- `CreatePersonResult` / `RenamePersonResult` / `SetPersonAdminResult` / `DeletePersonResult { ..., errors: [PersonError!]! }` (all `{ person, errors }` or `{ people, errors }` shaped - see the schema)
...
`createRoom` ([CreateRoomHandler](impl/src/main/java/com/mootmaker/handler/CreateRoomHandler.java)), `updateRoom` ([UpdateRoomHandler](impl/src/main/java/com/mootmaker/handler/UpdateRoomHandler.java)) and `deleteRoom` ([DeleteRoomHandler](impl/src/main/java/com/mootmaker/handler/DeleteRoomHandler.java)):

| Error | Rule |
|---|---|
| `NameRequired` | `name` must not be null or blank |
| `CapacityTooLow` | `capacity` must be ≥ 2 |
| `RoomNotFound` | `updateRoom`/`deleteRoom` only: `id` must refer to an existing room |
| `RoomHasUpcomingMeetings` | `deleteRoom` only: at least one meeting from today onward is still booked in this room |

`updateMyName` ([UpdateMyNameHandler](impl/src/main/java/com/mootmaker/handler/UpdateMyNameHandler.java), self-only), `renamePerson` ([RenamePersonHandler](impl/src/main/java/com/mootmaker/handler/RenamePersonHandler.java), admin-only), `setPersonAdmin` ([SetPersonAdminHandler](impl/src/main/java/com/mootmaker/handler/SetPersonAdminHandler.java), admin-only) and `deletePerson` ([DeletePersonHandler](impl/src/main/java/com/mootmaker/handler/DeletePersonHandler.java), admin-only) - split from a single `updatePerson` specifically so a mutation that can act on anyone, or grant admin, is named as exactly that:

| Error | Rule |
|---|---|
| `NameRequired` | `updateMyName`/`renamePerson` only: `name` must not be null or blank |
| `NameAlreadyExists` | `createPerson` only: another person already has this name (case/whitespace-insensitive) |
| `PersonNotFound` | `renamePerson`/`setPersonAdmin`/`deletePerson` only: `id` must refer to an existing person |
| `NoLinkedPerson` | `updateMyName` only: the caller must have a linked Person to rename |
| `NoLinkedAccount` | `setPersonAdmin` only: the target has no linked Cognito account, so there is nothing for admin access to actually grant |
| `CannotRevokeOwnAdminAccess` | `setPersonAdmin` only: no one can remove their own admin access through this mutation |
| `CannotDeleteSelf` | `deletePerson` only: the caller targeted their own Person - use `deleteMyAccount` instead |
