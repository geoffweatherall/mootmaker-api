...
class CreateMeetingAcceptanceIT {
  ...
  void createdMeetingIsReturnedByMeetingsQuery() {
    ...
    final JsonNode roomResult =
        client.execute(
            CREATE_ROOM_MUTATION, Map.of("room", Map.of("name", roomName, "capacity", 5)));
    ...
    final JsonNode createMeetingResult =
        client.execute(
    ...
    LOG.info(
        "Reading the day back through the composite entry point to check the meeting is returned");
    final JsonNode workspace =
        client
            .execute(
                "query Workspace($dates: [String!]) { workspace(dates: $dates) { days { date"
                    + " meetings { "
                    + MEETING_FIELDS
                    + " } } } }",
                Map.of("dates", List.of(BookableDates.day().toString())))
            .get("workspace");
