 ...
 *
 * <p>Whether a lookup was <i>skipped</i> is not observable from outside the API, and deliberately
 * so - it is an optimisation, not a contract. What is observable, and what actually matters, is
 * that every selection still returns correct data. The alias case is the one worth having a
 * deployed test for: if the resolver decided an aliased name was unwanted and returned an id-only
 * object, GraphQL would null the non-null field and cascade the null up through the list to the
 * meeting. That failure cannot happen in a unit test of the selection logic alone, because it needs
 * a real GraphQL executor enforcing non-null.
 */
class SelectionAwareMeetingsAcceptanceIT {

  private static final Logger LOG =
      LoggerFactory.getLogger(SelectionAwareMeetingsAcceptanceIT.class);
  ...
  static void createOneMeetingToInspect() {
    ...
    LOG.info(
        "Booking one meeting in '{}' organised by '{}' with attendee '{}'",
        roomName,
        organiserName,
        attendeeName);
   ...
   * meeting. The old {@code meetings(filter:)} field is gone: a date range is a list of day keys
   * now.
   */
  private JsonNode theMeeting(final String meetingSelection) {
    ...
    final JsonNode days =
        client
            .execute(query, Map.of("dates", List.of(BookableDates.day().toString())))
            .get("workspace")
            .get("days");
    assertThat(
        "one date was asked for, so exactly one day must come back", days.size(), equalTo(1));
  ...
  @Test
  @DisplayName(
      "ALIASED names still resolve - the case that would otherwise null a non-null field and"
          + " cascade")
