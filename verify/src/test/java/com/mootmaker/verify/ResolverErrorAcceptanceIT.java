 ...
 *
 * <pre>Cannot return null for non-nullable type: 'Workspace' within parent 'Query' (/workspace)</pre>
 *
...
class ResolverErrorAcceptanceIT {
    ...
    @Test
    @DisplayName("a resolver's own message reaches the client, rather than a generic non-nullable complaint")
    void surfacesTheResolversMessage() {
        final List<String> tooManyDates = IntStream.range(0, OVER_THE_DATE_LIMIT)
                .mapToObj(offset -> LocalDate.now().plusDays(offset).toString())
                .toList();

        final IllegalStateException thrown = assertThrows(IllegalStateException.class,
                () -> CLIENT.execute("query TooManyDates($dates: [String!]) { workspace(dates: $dates) { "
                        + "days { date } } }", Map.of("dates", tooManyDates)));
    ...
    void doesNotReportOnlyTheNullabilityError() {
        ...
        assertThat("the only error present was the nullability one",
                withoutTheRealReason, not(containsString("Too many dates requested")));
        assertThat(thrown.getMessage(), containsString("Too many dates requested"));
    }
}
