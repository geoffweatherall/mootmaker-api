...
class SuggestRoomAcceptanceIT {
  ...
  private static final String SUGGEST_ROOM_QUERY =
      "query SuggestRoom($startTime: String!, $endTime: String!, $requiredCapacity: Int!) {"
          + " suggestRoom(startTime: $startTime, endTime: $endTime, requiredCapacity:"
          + " $requiredCapacity) { id name capacity } }";
  ...
  private void bookMeeting(
      final String roomId, final String organiserId, final String startTime, final String endTime) {
    final JsonNode result =
        client.execute(
  ...
  private JsonNode suggestRoom(
      final String startTime, final String endTime, final int requiredCapacity) {
  ...
  void ranksEveryFreeRoomWithSufficientCapacitySmallestSurplusFirst() {
    ...
    LOG.info(
        "Creating rooms '{}' (capacity 3) and '{}' (capacity 6)", smallRoomName, largeRoomName);
