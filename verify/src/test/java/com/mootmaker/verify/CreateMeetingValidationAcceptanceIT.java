...
/**
 * Acceptance tests for the {@code createMeeting} validation rules: 15-minute time boundaries, room
 * capacity, and overlapping meetings for the same room (with overlap edge cases).
 */
class CreateMeetingValidationAcceptanceIT {

  private static final Logger LOG =
      LoggerFactory.getLogger(CreateMeetingValidationAcceptanceIT.class);
  ...
  void resetDatabaseAndCreateFixtures() {
    ...
    LOG.info(
        "Fixtures ready: room '{}' ({}), organiser {}, attendee {}",
        roomName,
        roomId,
        organiserId,
        attendeeId);
  ...
  void startTimeNotOnFifteenMinuteBoundaryIsRejected() {
    ...
    final JsonNode payload =
        createMeetingPayload(
            ...
            organiserId,
            List.of(attendeeId),
            BookableDates.at("10:20:00"),
            BookableDates.at("10:45:00"));
  ...
  void endTimeNotOnFifteenMinuteBoundaryIsRejected() {
    ...
    // :20 is on a 5-minute boundary but not a 15-minute one - see
    // startTimeNotOnFifteenMinuteBoundaryIsRejected.
    final JsonNode payload =
        createMeetingPayload(
            roomId,
            organiserId,
            List.of(attendeeId),
            BookableDates.at("10:00:00"),
            BookableDates.at("10:20:00"));
  ...
  void startTimeWithNonZeroSecondsIsRejected() {
    LOG.info(
        "Checking startTime with non-zero seconds is rejected even though the minute is on a 15"
            + " minute boundary");
    final JsonNode payload =
        createMeetingPayload(
            roomId,
            organiserId,
            List.of(attendeeId),
            BookableDates.at("10:00:30"),
            BookableDates.at("10:30:00"));
  ...
  void organiserAlsoListedAsAttendeeIsRejected() {
    ...
    final JsonNode payload =
        createMeetingPayload(
            roomId,
            organiserId,
            List.of(organiserId, attendeeId),
            BookableDates.at("10:00:00"),
            BookableDates.at("10:30:00"));
  ...
  void organiserAlsoListedAsAttendeeDoesNotDoubleCountTowardsCapacity() {
    LOG.info(
        "Checking the organiser-is-attendee case is rejected on its own merits, not reported as"
            + " InsufficientCapacity even though the naive '1 + attendeeIds.size()' count would"
            + " exceed the room");
    // ROOM_CAPACITY people total (organiser + the other (ROOM_CAPACITY - 1) attendees), but
    // organiserId is
    // also duplicated into attendeeIds, so a naive count would see ROOM_CAPACITY + 1 and wrongly
    // also raise
  ...
  void newMeetingOverlappingStartOfExistingMeetingIsRejected() {
    ...
    LOG.info(
        "Checking a new meeting that starts before and ends during an existing meeting is"
            + " rejected");
  ...
  void newMeetingOverlappingEndOfExistingMeetingIsRejected() {
    ...
    LOG.info(
        "Checking a new meeting that starts during and ends after an existing meeting is rejected");
  ...
  void meetingImmediatelyAfterExistingMeetingIsAllowed() {
    ...
    LOG.info(
        "Checking a new meeting starting exactly when an existing meeting ends is allowed"
            + " (back-to-back)");
  ...
  void meetingImmediatelyBeforeExistingMeetingIsAllowed() {
    ...
    LOG.info(
        "Checking a new meeting ending exactly when an existing meeting starts is allowed"
            + " (back-to-back)");
  ...
  void multipleValidationErrorsAreAllReturnedTogetherInOneRequest() {
    LOG.info(
        "Checking that unrelated validation failures (missing organiser, insufficient capacity,"
            + " unavailable time range) are all reported together rather than stopping at the first"
            + " one");
    createExistingMeeting(BookableDates.at("10:00:00"), BookableDates.at("11:00:00"));
    ...
    // organiserId does not exist; attendee count exceeds room capacity; time range overlaps the
    // existing meeting.
    final JsonNode payload =
        createMeetingPayload(
            roomId,
  ...
  private void createExistingMeeting(final String startTime, final String endTime) {
    ...
    final JsonNode payload =
        createMeetingPayload(roomId, organiserId, List.of(attendeeId), startTime, endTime);
  ...
  private String createRoom(final String name, final int capacity) {
    final JsonNode result =
        client.execute(
            CREATE_ROOM_MUTATION, Map.of("room", Map.of("name", name, "capacity", capacity)));
  ...
  private JsonNode createMeetingPayload(
      final String roomId,
