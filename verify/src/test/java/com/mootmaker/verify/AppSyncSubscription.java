 ...
 *
 * <p>Publishing before {@code start_ack} is a real race and the reason {@link #awaitReady()} exists:
 * a broadcast sent between {@code start} and {@code start_ack} is not delivered, and the test would
 * fail for a reason that has nothing to do with the code under test.
 *
 * <p>The realtime URL is derived from {@code GRAPHQL_API_URL} rather than exported separately, but
 * the path differs from the host's: on a custom domain the realtime endpoint is
 * {@code /graphql/realtime}, where the raw AppSync realtime host serves {@code /graphql}. Verified
 * both ways - {@code /graphql} on the custom domain fails to connect at all.
 */
final class AppSyncSubscription implements AutoCloseable {
    ...
    private AppSyncSubscription(final String query, final String endpoint, final String accessToken) {
        ...
        final URI realtime = URI.create("wss://" + host + http.getPath() + "/realtime"
                + "?header=" + encodedHeader + "&payload=" + base64(Map.of()));
    ...
    static AppSyncSubscription open(final String query, final String endpoint, final String accessToken) {
    ...
    void awaitReady() throws InterruptedException {
        if (!ready.await(READY_TIMEOUT.toSeconds(), TimeUnit.SECONDS)) {
            throw new IllegalStateException("AppSync never acknowledged the subscription within "
                    + READY_TIMEOUT + ". Protocol errors seen: " + protocolErrors);
        }
    }
    ...
    private final class Listener implements WebSocket.Listener {
        ...
        private void handle(final WebSocket webSocket, final JsonNode message) throws Exception {
            switch (message.path("type").asText()) {
                case "connection_ack" -> {
                    final Map<String, Object> start = Map.of(
                            "id", UUID.randomUUID().toString(),
                            "type", "start",
                            "payload", Map.of(
                                    "data", MAPPER.writeValueAsString(Map.of("query", query, "variables", Map.of())),
                                    "extensions", Map.of("authorization",
