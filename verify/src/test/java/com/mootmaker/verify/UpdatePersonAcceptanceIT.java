...
/**
 * Acceptance tests for updating a person's name and reading the change back via the {@code people}
 * query. The acceptance-test client authenticates as the M2M tooling client, which carries the
 * admin-equivalent OAuth scope (see the API README's "M2M tooling" section), so every person it
 ...
 * to authenticate as; those are covered by {@code UpdatePersonHandlerTest} instead.
 */
class UpdatePersonAcceptanceIT {
  ...
  private static final String UPDATE_PERSON_MUTATION =
      "mutation UpdatePerson($id: ID!, $person: PersonInput!) { updatePerson(id: $id, person:"
          + " $person) { person { id name } errors } }";
  ...
  void updatedPersonIsReturnedByPeopleQuery() {
    ...
    final JsonNode updateResult =
        client.execute(
            UPDATE_PERSON_MUTATION, Map.of("id", personId, "person", Map.of("name", newName)));
    ...
    final JsonNode updateResult =
        client.execute(
            ...
            Map.of(
                ...
                "person",
                Map.of("name", "Anything")));

    final JsonNode updatePersonPayload = updateResult.get("updatePerson");
    assertThat(updatePersonPayload.get("person").isNull(), is(true));
    assertThat(
        updatePersonPayload.get("errors").get(0).asText(),
  ...
  void updatingWithABlankNameIsRejected() {
    ...
    final JsonNode updateResult =
        client.execute(
            UPDATE_PERSON_MUTATION, Map.of("id", personId, "person", Map.of("name", "   ")));
