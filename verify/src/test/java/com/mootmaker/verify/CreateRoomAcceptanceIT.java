...
/**
 * Acceptance tests for creating a room, reading it back via the {@code rooms} query, and
 * validation.
 */
class CreateRoomAcceptanceIT {
  ...
  void createdRoomIsReturnedByRoomsQuery() {
    ...
    final JsonNode createResult =
        client.execute(
            CREATE_ROOM_MUTATION, Map.of("room", Map.of("name", roomName, "capacity", capacity)));
  ...
  void blankRoomNameIsRejected() {
    ...
    assertThat(
        createRoomPayload.get("errors").get(0).asText(), equalTo(RoomError.NameRequired.name()));
  }
  ...
  void capacityOfZeroIsRejected() {
    ...
    assertThat(createRoomPayload.get("room").isNull(), is(true));
    assertThat(
        createRoomPayload.get("errors").get(0).asText(), equalTo(RoomError.CapacityTooLow.name()));
  }
