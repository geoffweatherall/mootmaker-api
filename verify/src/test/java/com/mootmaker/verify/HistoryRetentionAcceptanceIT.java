...
class HistoryRetentionAcceptanceIT {
   ...
   * (Limits.RETENTION_DAYS_MINIMUM) rather than a shared constant that could drift with it
   * silently. If the two disagree, the assertions below fail - which is the point.
   */
  ...
  private static String boundary() {
    return client
        .execute(WORKSPACE_QUERY, Map.of("dates", List.of()))
        .get("workspace")
        .get("boundaries")
        .get("earliestRetainedDate")
        .asText();
  }
        ...
        client.execute(
            ...
            Map.of(
                ...
                Map.of(
                    ...
                    date + "T09:30:00")));
    assertThat(
        "seeding " + date + " must be accepted - past bookings stay legal for this reason",
  ...
  @Test
  @DisplayName(
      "deletes history before the boundary, keeps the day ON it, and leaves later days untouched")
  void deletesOnlyWhatIsPastTheBoundary() {
    ...
    assertThat(
        "precondition: all three days hold a meeting",
        datesHoldingMeetings(seeded).size(),
        equalTo(3));
    ...
    assertThat(
        "the API must now report what the job advanced to", boundary(), equalTo(newBoundary));
    ...
    assertThat(
        "every surviving day must be on or after the boundary",
        surviving,
        everyItem(greaterThanOrEqualTo(newBoundary)));
  }
  ...
  void dryRunChangesNothing() {
    ...
    assertThat(
        "the day must still be there",
        datesHoldingMeetings(List.of(willExpire)),
        hasItem(willExpire.toString()));
  }
}
