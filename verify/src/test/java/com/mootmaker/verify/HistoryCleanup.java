 ...
 *
 * <p>Invoked rather than waited for on purpose. The EventBridge rule is deliberately disabled
 * outside {@code test} and {@code production}, because a schedule firing mid-run would make these
 * tests nondeterministic - so the job runs here only when a test asks it to.
 */
