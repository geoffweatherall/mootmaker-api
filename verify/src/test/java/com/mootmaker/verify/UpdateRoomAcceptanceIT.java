 ...
 * rooms} query, and validation. The acceptance-test client authenticates as the M2M tooling client,
 * which carries the admin-equivalent OAuth scope (see the API README's "M2M tooling" section) -
 * there's no way to exercise a non-admin rejection from this suite, since it has no real signed-in
 * user to authenticate as; that's covered by {@code UpdateRoomHandlerTest} instead.
 */
class UpdateRoomAcceptanceIT {
  ...
  private static final String UPDATE_ROOM_MUTATION =
      "mutation UpdateRoom($id: ID!, $room: RoomInput!) { updateRoom(id: $id, room: $room) { room {"
          + " id name capacity } errors } }";
  ...
  void updatedRoomIsReturnedByRoomsQuery() {
    ...
    final JsonNode updateResult =
        client.execute(
            ...
            Map.of("id", roomId, "room", Map.of("name", newName, "capacity", 12)));
    ...
    final JsonNode roomsResult =
        client.execute("query { workspace { rooms { id name capacity } } }");
    ...
    final JsonNode updateResult =
        client.execute(
            ...
            Map.of(
                ...
                "room",
                Map.of("name", "Anything", "capacity", 5)));

    final JsonNode updateRoomPayload = updateResult.get("updateRoom");
    assertThat(updateRoomPayload.get("room").isNull(), is(true));
    assertThat(
        updateRoomPayload.get("errors").get(0).asText(), equalTo(RoomError.RoomNotFound.name()));
  }
  ...
  void updatingWithABlankNameIsRejected() {
    ...
    final JsonNode updateResult =
        client.execute(
            UPDATE_ROOM_MUTATION,
            Map.of("id", roomId, "room", Map.of("name", "   ", "capacity", 8)));

    final JsonNode updateRoomPayload = updateResult.get("updateRoom");
    assertThat(updateRoomPayload.get("room").isNull(), is(true));
    assertThat(
        updateRoomPayload.get("errors").get(0).asText(), equalTo(RoomError.NameRequired.name()));
  }
}
