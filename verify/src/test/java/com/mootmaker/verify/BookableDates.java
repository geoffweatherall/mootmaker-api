 ...
 *
 * <p>Meetings may only be created between the stored retention boundary and the booking horizon, and
 * the retention boundary <b>moves</b> - it advances one Monday every week. A hardcoded date is
 * therefore not a constant at all: it is a date that silently falls out of the bookable window some
 * weeks after it was written, failing a test that has nothing to do with dates. Three of these tests
 * had already crossed that line by the time the rule was enforced.
 *
