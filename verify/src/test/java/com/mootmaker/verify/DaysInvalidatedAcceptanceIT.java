...
class DaysInvalidatedAcceptanceIT {
  ...
  /**
   * Generous on purpose. The publish is an extra network hop made by a Lambda that may be cold, and
   * a broadcast that is merely slow is not the failure this test is looking for.
   */
  private static final Duration DELIVERY_TIMEOUT = Duration.ofSeconds(20);

  /**
   * How long to wait before concluding nothing was broadcast. Shorter than DELIVERY_TIMEOUT because
   * it is paid in full by every run: this wait always expires, it never completes early.
   */
  ...
  private static Map<String, Object> meeting(
      final String date, final String from, final String to) {
    return Map.of(
        "roomId",
        roomId,
        "organiserId",
  ...
  void broadcastsACreatedMeeting() throws Exception {
    ...
    try (AppSyncSubscription subscription =
      ...
      final JsonNode result =
          client.execute(CREATE_MEETING, Map.of("meeting", meeting(date, "09:00:00", "09:30:00")));
      assertThat(
          "precondition: the booking must have been accepted",
          result.get("createMeeting").get("errors").size(),
          equalTo(0));
    ...
    try (AppSyncSubscription subscription =
      ...
      final JsonNode result =
          client.execute(
              ...
              Map.of(
                  "meeting",
                  meeting(date, "09:00:00", "09:30:00").entrySet().stream()
                      .collect(
                          HashMap::new,
                          (m, e) ->
                              m.put(
                                  e.getKey(),
                                  e.getKey().equals("roomId") ? "no-such-room" : e.getValue()),
                          HashMap::putAll)));
      assertThat(
          "precondition: the booking must have been REJECTED",
          result.get("createMeeting").get("errors").size(),
          org.hamcrest.Matchers.greaterThan(0));

      assertThat(
          "a booking that was refused changed no day, so nothing may be broadcast",
          subscription.receivedNothingWithin(SILENCE_TIMEOUT),
          equalTo(true));
    }
  }
  ...
  void broadcastsOncePerBulkCall() throws Exception {
    ...
    try (AppSyncSubscription subscription =
      ...
      assertThat(
          "precondition: every meeting must have been accepted",
          result.get("createMeetings").get("failures").size(),
          equalTo(0));

      final JsonNode first = subscription.awaitMessage(DELIVERY_TIMEOUT);
      assertThat(
          "no broadcast arrived. Protocol errors: " + subscription.protocolErrors(),
          first,
          org.hamcrest.Matchers.notNullValue());
      ...
      assertThat(
          "four meetings on one date must not produce four broadcasts",
          subscription.receivedNothingWithin(SILENCE_TIMEOUT),
          equalTo(true));
    }
  }
